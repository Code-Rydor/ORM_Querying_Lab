...
def problem_two(request):
    ...
    order_by = instructors.order_by('hire_date')
    for instructor in order_by:
...
def problem_three(request):
  ...
  print(f'{instructors.first_name} {instructors.last_name}')
...
def problem_four(request):
    ...
    print(f'Students Count: {count_students}\nCourses Count: {count_courses}\nInstructors Count: {count_instructors}')
...
def problem_five(request):
    ...
    #student = Student.objects.create(first_name='Garrett', last_name='Landin', year=9, gpa=4.0)
...
def problem_six(request):
    ...
    student_id = 11
    Student.objects.filter(pk=11).update(gpa=3.9)
